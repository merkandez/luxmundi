...
const PaginationButton = ({ direction }) => {
  ...
  const ButtonWrapper = styled.button.attrs((props) => ({
    'aria-pressed': props.$isActive, // Evita que `isActive` se pase al DOM directamente
  }))`
  display: flex;
  align-items: center;
  gap: 8px;
  ...
  padding: 8px 12px;
  background: ${(props) => (props.isActive ? 'blue' : 'none')}; /* Cambia color si es activo */
